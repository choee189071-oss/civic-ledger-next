...
function compactRecord(record: any, templateKey: ReportTemplate) {
  ...
  return {
    ...
    sourceCoverageMatrix: record?.evidencePackage?.source_coverage_matrix ?? [],
    outputEligibility: record?.evidencePackage?.output_eligibility ?? null,
    uploadQueue: record?.evidencePackage?.upload_queue ?? [],
    sourceEvidence: evidenceSources.map((result: any) => ({
      ...
      accessStatus: result.accessStatus ?? result.access_status,
      recommendedAction: result.recommendedAction ?? result.recommended_action,
      notes: result.notes,
    })),
...
function reportInstructions(template: (typeof REPORT_TEMPLATES)[ReportTemplate], templateKey: ReportTemplate) {
  ...
  return [
    ...
    'Preserve the recency policy from the research package. Separate quarterly updates, one-year monitoring evidence, and three-year structural context.',
    ...
    'Use outputEligibility.selected when supplied. If the selected output status is Needs Sources, write the deliverable as a source-gap / intake report, not a final memo.',
    'Use sourceCoverageMatrix when supplied to show Found, Needs Upload, Manual Verification Required, Stale, and Good Enough for Output status by evidence area.',
    'Use uploadQueue when supplied to list the exact documents the analyst should upload through Important Files / LlamaParse, including preferred document type and why it matters.',
    'If EMMA/MSRB or rating-agency evidence is marked Manual Verification Required, treat it as source metadata only until the underlying PDF or public document is uploaded or verified.',
